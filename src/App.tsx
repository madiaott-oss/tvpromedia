...
import { 
  ...
  ESPOIR_TV_LOGO, BALADE_MONDE_LOGO, CENTRAL_VOICE_LOGO, METEO_TV_LOGO, MSTV_LOGO, MS_RADIO_LOGO, NURU_TV_LOGO, SM_VIDEO_TV_LOGO, TVLB_LOGO,
  LA_BORNE_MPASA_LOGO, OCEAN_TV_LOGO, DIESKOLUS_TV_LOGO, PAROLE_DESPERANCE_LOGO, A_LA_UNE_TELEVISION_LOGO,
...
const normalizeCategory = (cat?: string): Channel['cat'] => {
  const v = (cat || '').toUpperCase().trim();
  if (v.includes('SPORT')) return 'SPORTS';
  if (v.includes('MOVIE') || v.includes('FILM') || v.includes('SERIE')) return 'FILMS';
  if (v.includes('NEWS') || v.includes('ACTU') || v.includes('INFO')) return 'NEWS';
  if (v.includes('RELIG') || v.includes('FOI')) return 'RELIGIEUX';
  if (v.includes('DOC') || v.includes('EDU') || v.includes('SCIENCE') || v.includes('CULTURE')) return 'DOCUMENTAIRE';
  if (v.includes('MUSIC') || v.includes('MUSIQ') || v.includes('RELAX')) return 'MUSIQUE';
  if (v.includes('ENFANT') || v.includes('KID') || v.includes('ANIM')) return 'ENFANTS';
  if (v.includes('RADIO')) return 'RADIO';
  if (v.includes('METEO') || v.includes('WEATHER')) return 'METEO';
  return 'GENERALISTE';
};
...
const deduplicateChannels = (channelList: Channel[]): Channel[] => {
  ...
  return channelList.filter(ch => {
    ...
    // Guarantee valid normalized category
    ch.cat = normalizeCategory(ch.cat || (ch as any).categorie);

    // Resolve unified stream URL fallback
    ...
    // Explicitly suppressed channels per user request (OASIS RADIO, ALLIANCE MABANZA TV, COP TELEVISION, TROMPETTE MEDIA, ACK TV, GRACE TV, CCPV MONTREAL CANAL 23)
    if (
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (cachedCh) {
      try {
        ...
        const REMOVED_CH_IDS = new Set([
          ...
          'ch_mabanza', '33', 'ch_96', '96', 'ch_116', '116',
          'ch_trompette', '12',
          'ch_72', '72', 'ch_gracetv', '29', 'ch_23', '23'
        ]);
        const REMOVED_NAMES = [
          ...
          "FILMS D'ACTION RAKUTEN", 'FILMS D ACTION RAKUTEN', '13 CULTURA',
          'OASIS RADIO', 'OASIS MEDIA FM', 'OASIS MEDIA', 'RADIO OASIS', 'OASIS FM',
          'ALLIANCE MABANZA TV', 'ALLIANCE MABANZA', 'MABANZA',
          'COP TELEVISION', 'COP TV',
          'TROMPETTE MEDIA', 'TROMPETTE', 'TROMPETE MEDIA', 'TROMPETE',
          'ACK TV', 'ACK TELEVISION', 'GRACE TV',
          'CCPV TV MONTRÉAL (SECOURS)', 'CCPV TV MONTREAL (SECOURS)'
        ];
        ...
        parsed = parsed.filter(ch => {
          ...
          if (upName === 'ACK' || upName.includes('ACK TV') || upName.includes('GRACE TV')) return false;
          if (ch.ch === '23' && (upName.includes('CCPV') || upName.includes('PAROLE DE VIE'))) return false;
        ...
        parsed = parsed.map(ch => {
          if ((ch.id === 'ch_22' || ch.id === 'ch_24' || ch.id === 'ch_30') && ch.cat === 'RELIGIEUX' as any) {
            migrated = true;
            ch = { ...ch, cat: 'RELIGIEUX' };
          }
          ...
          if (ch.id === 'ch_rtvradio' || (ch.nom && (ch.nom.trim().toUpperCase() === 'RTP RADIO' || ch.nom.trim().toUpperCase() === 'RTV RADIO')) || ch.ch === '6') {
            if (ch.nom !== 'RTP RADIO' || ch.logo !== RTP_RADIO_LOGO || ch.lien !== 'https://www.tvpromedia.com/live/cle_rtvradio_1m_xxmm.m3u8' || ch.ch !== '6') {
              migrated = true;
              ch = {
                ...ch,
                id: 'ch_rtvradio',
          ...
          if (ch.id === 'ch_news234' || (ch.nom && (ch.nom.trim().toUpperCase() === 'NEWS +243 RDC TV' || ch.nom.trim().toUpperCase() === 'NEWS +243' || ch.nom.trim().toUpperCase() === 'NEWS 243 RDC TV' || ch.nom.trim().toUpperCase() === 'NEWS 243')) || ch.ch === '7') {
            if (ch.nom !== 'NEWS +243 RDC TV' || ch.logo !== NEWS_243_RDC_LOGO || ch.lien !== 'https://www.tvpromedia.com/live/cle_news234_1m_jgx9.m3u8' || ch.ch !== '7') {
              migrated = true;
              ch = {
                ...ch,
                id: 'ch_news234',
          ...
          if (ch.id === 'ch_24' || ch.id === '24' || ch.ch === '24' || (ch.nom && (ch.nom.toUpperCase().includes('13 CULTURA') || ch.nom.toUpperCase().includes('CULTURA')))) {
            const targetCcpvStream = 'https://www.tvpromedia.com/live/cle_ccpvtv_1m_9miq.m3u8';
            migrated = true;
            const { youtubeBackup: _unused, ...rest } = ch;
            ch = { 
              ...rest, 
              id: 'ch_24',
              ch: '24',
          ...
          if (ch.id === 'ch_28' || (ch.nom && ch.nom.toUpperCase().includes('BALADE'))) {
            if (ch.logo !== BALADE_MONDE_LOGO || ch.lien !== 'https://www.tvpromedia.com/live/cle_baladetv_inf_gx5l.m3u8' || ch.m3u8Source !== 'https://stream.berosat.live:19360/live-tv/live-tv.m3u8' || ch.cloudRemix !== 'https://stream.berosat.live:19360/live-tv/live-tv.m3u8' || ch.nom === 'BALADE MONDE TV ') {
              migrated = true;
              ch = { 
                ...ch, 
                nom: 'BALADE MONDE TV',
          ...
          if (ch.id === 'ch_31' || (ch.nom && ch.nom.toUpperCase() === 'MSTV')) {
            if (ch.logo !== MSTV_LOGO || ch.lien !== 'https://ip-pro.berosat.live/hls/live/MSTV/index.m3u8' || ch.m3u8Source !== 'https://ip-pro.berosat.live/hls/live/MSTV/index.m3u8') {
              migrated = true;
              ch = { 
                ...ch, 
                logo: MSTV_LOGO,
          ...
          if (ch.id === 'ch_390' || (ch.nom && ch.nom.toUpperCase() === 'MS RADIO')) {
            if (ch.logo !== MS_RADIO_LOGO || ch.lien !== 'https://ip-pro.berosat.live/hls/live/MSTV/index.m3u8' || ch.m3u8Source !== 'https://ip-pro.berosat.live/hls/live/MSTV/index.m3u8') {
              migrated = true;
              ch = {
                ...ch,
                id: 'ch_390',
          ...
          if (ch.id === 'ch_66' || (ch.nom && ch.nom.toUpperCase().includes('HORIZON 2000'))) {
            if (ch.logo !== HORIZON_2000_LOGO || ch.lien !== 'https://www.tvpromedia.com/live/cle_horizontv_12m_c0wv.m3u8' || ch.m3u8Source !== 'https://www.tvpromedia.com/live/cle_horizontv_12m_c0wv.m3u8') {
              migrated = true;
              ch = { 
                ...ch, 
                nom: "HORIZON 2000 TV",
          ...
          if (ch.id === 'ch_rtp' || (ch.nom && ch.nom.trim().toUpperCase() === 'RTP')) {
            if (ch.lien !== 'https://www.tvpromedia.com/live/cle_rtptv_1m_u4tx.m3u8') {
              migrated = true;
              ch = {
                ...ch,
                nom: "RTP",
          ...
          // Re-route any old placeholder domain or insecure HTTP VPS link to direct HTTPS live stream
          if (ch.lien && (ch.lien.includes('tvpromedia.ai.studio/live/') || ch.lien.includes('191.215.38.95:8080/live/'))) {
            migrated = true;
            const streamKey = ch.rtmpKey || ch.lien.split('/live/')[1];
            ch = { ...ch, lien: `https://www.tvpromedia.com/live/${streamKey}`, m3u8Source: `https://www.tvpromedia.com/live/${streamKey}` };
          }
          if (ch.m3u8Source && (ch.m3u8Source.includes('tvpromedia.ai.studio/live/') || ch.m3u8Source.includes('191.215.38.95:8080/live/'))) {
            migrated = true;
            const streamKey = ch.rtmpKey || ch.m3u8Source.split('/live/')[1];
            ch = { ...ch, m3u8Source: `https://www.tvpromedia.com/live/${streamKey}` };
          }
          return ch;
        });
        ...
        // Absolute guarantee that Canal 24 (CCPV TV MONTRÉAL) is present with the correct stream & name
        const idx24 = parsed.findIndex(c => c.id === 'ch_24' || c.ch === '24');
        const ccpv24Stream = 'https://www.tvpromedia.com/live/cle_ccpvtv_1m_9miq.m3u8';
        if (idx24 === -1) {
          parsed.push({
            id: 'ch_24',
            nom: 'CCPV TV MONTRÉAL',
            lien: ccpv24Stream,
            m3u8Source: ccpv24Stream,
            cloudRemix: ccpv24Stream,
            rtmpUrl: 'rtmp://191.215.38.95/live',
            rtmpKey: 'cle_ccpvtv_1m_9miq',
            logo: CCPV_TV_LOGO,
            cat: 'RELIGIEUX',
          ...
          const current24 = parsed[idx24];
          if (current24.nom !== 'CCPV TV MONTRÉAL' || current24.id !== 'ch_24' || current24.ch !== '24' || current24.lien !== ccpv24Stream || current24.logo !== CCPV_TV_LOGO) {
            parsed[idx24] = {
              ...current24,
              id: 'ch_24',
              ch: '24',
    ...
    // Background automatic catalog synchronization from /channels.json and /api/channels with cache buster
    const syncCatalog = async () => {
      try {
        let serverList: Channel[] | null = null;
        const resJson = await fetch(`/channels.json?t=${Date.now()}`, { cache: 'no-cache' });
        if (resJson.ok) {
  ...
  const handlePlayChannel = (ch: Channel) => {
    ...
    const channelToPlay: Channel = {
      ...ch,
      lien: ch.lien || streamUrl || '',
      url: ch.url || streamUrl || undefined,
      src: ch.src || streamUrl || undefined,
    };
    ...
    <div className="min-h-screen bg-[#060813] text-white flex flex-col selection:bg-red-600 selection:text-white" id="main-application-wrapper">
      ...
      <section className="bg-black relative" id="cloud-streaming-stage">
        {appPublished && !paymentVerified ? (
          ...
          (() => {
            const streamUrl = activeChannel ? (activeChannel.lien || activeChannel.m3u8Source || activeChannel.url || activeChannel.src) : null;
            return (
              <VideoPlayer 
                src={streamUrl}
                lien={activeChannel?.lien ?? null}
              ...
              >
                ...
                {channels.length > 0 && (
                  <span className={`px-2 py-0.5 text-[10px] rounded-full font-black ${
                    activeTab === 'tout' && !selectedCategory ? 'bg-black/40 text-white' : 'bg-white/10 text-gray-300'
                  }`}>
                    {channels.length}
                  </span>
                )}
            ...
            {CATEGORIES.map(categoryName => {
              ...
              return (
                <section key={categoryName} id={`category-sec-${categoryName}`} className="space-y-4">
                  <div className="flex items-center justify-between border-b border-white/[0.05] pb-2 flex-wrap gap-2">
                    ...
                    {hasMore && !isSearchingOrFiltering && (
                      <button
                        type="button"
                  ...
                  {/* Load more controls for large categories */}
                  {hasMore && !isSearchingOrFiltering && (
                    <div className="flex justify-center items-center gap-3 pt-3">
                      <button
                        ...
                        onClick={() => setExpandedCategories(prev => ({
                          ...prev,
                          [categoryName]: (prev[categoryName] || 48) + 96
                        }))}
                        className="px-4 py-2 bg-[#141b2f] hover:bg-red-600 hover:text-white text-gray-300 font-bold text-xs rounded-xl border border-white/10 transition-all shadow-md"
                      >
                        Charger 96 chaînes suivantes ({catChan.length - limit} restantes)
                      </button>
                      <button
                        type="button"
                        onClick={() => setExpandedCategories(prev => ({
                          ...prev,
                          [categoryName]: catChan.length
                        }))}
                        className="px-3 py-2 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white text-xs rounded-xl border border-white/5 transition-all"
                      >
                        Tout afficher ({catChan.length})
                      </button>
                    </div>
                  )}
                </section>
