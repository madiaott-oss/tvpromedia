...
import { getEpgLogo } from '../data/epgLogos';
...
export default function AdminPanel({ channels, viewCounts, onUpdateChannels, onResetToDefaults, onClose }: AdminPanelProps) {
  ...
  const [m3uSelectedCat, setM3uSelectedCat] = useState('AUTO');
  ...
  const [genM3u8Url, setGenM3u8Url] = useState<string>(() => `https://www.tvpromedia.com/live/stream.m3u8`);
  ...
  const [partnerLicenses, setPartnerLicenses] = useState<PartnerLicense[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        m3u8Url: 'https://www.tvpromedia.com/live/cle_congo_1m_cl0b.m3u8',
        durationMonths: 12,
        issuedAt: today.toISOString().split('T')[0],
        expiresAt: d3.toISOString().split('T')[0],
      ...
      },
      {
        ...
        m3u8Url: 'https://www.tvpromedia.com/live/cle_rtvradio_1m_xxmm.m3u8',
        durationMonths: 12,
        issuedAt: today.toISOString().split('T')[0],
        expiresAt: d3.toISOString().split('T')[0],
      ...
      },
      {
        ...
        m3u8Url: 'https://www.tvpromedia.com/live/cle_news234_1m_jgx9.m3u8',
        durationMonths: 12,
        issuedAt: today.toISOString().split('T')[0],
        expiresAt: d3.toISOString().split('T')[0],
      ...
      },
      {
        ...
        m3u8Url: 'https://www.tvpromedia.com/live/cle_malaika_1m_vllq.m3u8',
        durationMonths: 1,
        issuedAt: today.toISOString().split('T')[0],
      ...
      {
        id: 'lic-ccpv',
        partnerName: 'Centre Chrétien Parole de Vie (CCPV TV Montréal)',
        channelId: 'ch_24',
        channelName: 'CCPV TV MONTRÉAL (Canal 24)',
        contact: 'Direction CCPV TV Montréal',
        rtmpUrl: 'rtmp://191.215.38.95/live',
        streamKey: 'cle_ccpvtv_1m_9miq',
        m3u8Url: 'https://www.tvpromedia.com/live/cle_ccpvtv_1m_9miq.m3u8',
        durationMonths: 12,
        issuedAt: today.toISOString().split('T')[0],
        expiresAt: d3.toISOString().split('T')[0],
        notes: 'Chaîne Religieuse CCPV TV Montréal - Canal 24 & Canal 23 - Flux Principal & Secours HLS VPS cle_ccpvtv_1m_9miq sur www.tvpromedia.com',
      },
      {
        id: 'lic-1',
        ...
        m3u8Url: 'https://www.tvpromedia.com/live/cle_eglise_primitive_live.m3u8',
        durationMonths: 2,
        issuedAt: today.toISOString().split('T')[0],
      ...
      },
      {
        ...
        m3u8Url: 'https://www.tvpromedia.com/live/cle_cmc_tv_hd_live.m3u8',
        durationMonths: 1,
        issuedAt: today.toISOString().split('T')[0],
        expiresAt: d1.toISOString().split('T')[0],
      ...
      },
      {
        ...
        m3u8Url: 'https://www.tvpromedia.com/live/cle_prodige_tv_live_98x.m3u8',
        durationMonths: 3,
        issuedAt: today.toISOString().split('T')[0],
        expiresAt: d3.toISOString().split('T')[0],
...
=====================================================`;
  ...
  const applyVpsHostAndRegenerate = (host: string, port: string = srsHlsPort, newKey?: string) => {
    ...
    // RTMP ingest always points to VPS RTMP port 1935
    const rtmp = (cleanHost === 'www.tvpromedia.com' || cleanHost === 'tvpromedia.com')
      ? 'rtmp://191.215.38.95/live'
      : (cleanHost.includes('/') ? `rtmp://${cleanHost}` : `rtmp://${cleanHost}/live`);

    // In modern browsers, site is loaded via HTTPS (https://www.tvpromedia.com).
    // Direct https://www.tvpromedia.com causes Mixed-Content blocking.
    // The web server proxies /live/ to SRS, so HTTPS proxy on www.tvpromedia.com/live/ is used for web playback!
    const m3u8 = (cleanHost === '191.215.38.95' || cleanHost.includes('tvpromedia.com') || cleanHost.includes('tvpromedia.ai.studio'))
      ? `https://www.tvpromedia.com/live/${activeKey}.m3u8`
      : `http://${cleanHost}:${port || '8080'}/live/${activeKey}.m3u8`;
    
    setGenRtmpServer(rtmp);
    setGenM3u8Url(m3u8);
    triggerStatus(`📡 Configuration VPS RTMP & M3U8 HTTPS mise à jour pour : ${cleanHost}`);
  };
  ...
  const handleBulkRegenerateAllChannels = () => {
    ...
    if (confirm(`Voulez-vous vraiment régénérer automatiquement les clés RTMP et les URLs M3U8 pour TOUTES les ${channels.length} chaînes avec le serveur VPS [${activeHost}] ?`)) {
      const updated = channels.map((ch) => {
        ...
        const m3u8 = (activeHost.includes('tvpromedia.com') || activeHost === '191.215.38.95' || activeHost.includes('tvpromedia.ai.studio'))
          ? `https://www.tvpromedia.com/live/${key}.m3u8`
          : `http://${activeHost}:${activePort}/live/${key}.m3u8`;
        const rtmpUrl = (activeHost === 'www.tvpromedia.com' || activeHost === 'tvpromedia.com')
          ? 'rtmp://191.215.38.95/live'
          : `rtmp://${activeHost}/live`;
        return {
          ...ch,
          rtmpUrl,
          rtmpKey: key,
  ...
  const handleRegenerateSingleChannelKey = (channelId: string) => {
    ...
    const rtmpUrl = (activeHost === 'www.tvpromedia.com' || activeHost === 'tvpromedia.com')
      ? 'rtmp://191.215.38.95/live'
      : `rtmp://${activeHost}/live`;
    const hlsUrl = (activeHost.includes('tvpromedia.com') || activeHost === '191.215.38.95' || activeHost.includes('tvpromedia.ai.studio'))
      ? `https://www.tvpromedia.com/live/${newKey}.m3u8`
      : `http://${activeHost}:${activePort}/live/${newKey}.m3u8`;
  ...
  // The 14 official IPTV-ORG playlists requested by the user
  const IPTV_ORG_PRESETS = [
    { id: 'sports', name: 'Sports', icon: '⚽', url: 'https://iptv-org.github.io/iptv/categories/sports.m3u', cat: 'SPORTS' },
    { id: 'movies', name: 'Films & Séries', icon: '🎬', url: 'https://iptv-org.github.io/iptv/categories/movies.m3u', cat: 'FILMS' },
    { id: 'news', name: 'Actualités', icon: '📰', url: 'https://iptv-org.github.io/iptv/categories/news.m3u', cat: 'NEWS' },
    { id: 'education', name: 'Éducation', icon: '🎓', url: 'https://iptv-org.github.io/iptv/categories/education.m3u', cat: 'DOCUMENTAIRE' },
    { id: 'general', name: 'Généraliste', icon: '📺', url: 'https://iptv-org.github.io/iptv/categories/general.m3u', cat: 'GENERALISTE' },
    { id: 'relax', name: 'Relax / Détente', icon: '🧘', url: 'https://iptv-org.github.io/iptv/categories/relax.m3u', cat: 'MUSIQUE' },
    { id: 'religious', name: 'Religieux & Foi', icon: '🕊️', url: 'https://iptv-org.github.io/iptv/categories/religious.m3u', cat: 'RELIGIEUX' },
    { id: 'science', name: 'Sciences & Nature', icon: '🔬', url: 'https://iptv-org.github.io/iptv/categories/science.m3u', cat: 'DOCUMENTAIRE' },
    { id: 'weather', name: 'Météo Monde', icon: '☀️', url: 'https://iptv-org.github.io/iptv/categories/weather.m3u', cat: 'METEO' },
    ...
    { id: 'alz', name: 'Chaînes Alur (alz)', icon: '💬', url: 'https://iptv-org.github.io/iptv/languages/alz.m3u', cat: 'GENERALISTE' },
    { id: 'aii', name: 'Néo-Araméen (aii)', icon: '🗣️', url: 'https://iptv-org.github.io/iptv/languages/aii.m3u', cat: 'GENERALISTE' },
    { id: 'undefined', name: 'Divers / Indéfinies', icon: '🌐', url: 'https://iptv-org.github.io/iptv/categories/undefined.m3u', cat: 'GENERALISTE' },
  ];

  const handleImportFromUrl = async (targetUrl?: string, categoryOverride?: string) => {
    const finalUrl = (targetUrl || m3uUrlInput).trim();
    if (!finalUrl) {
      setM3uStatus({ success: false, msg: "Veuillez entrer ou sélectionner une URL M3U valide." });
      return;
    }

    setIsImportingUrl(true);
    setImportProgressMsg(`Connexion et téléchargement du flux : ${finalUrl}...`);
    setM3uStatus(null);

    try {
      const response = await fetch('/api/import-m3u-url', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          url: finalUrl,
          defaultCategory: categoryOverride && categoryOverride !== 'AUTO' ? categoryOverride : (m3uSelectedCat !== 'AUTO' ? m3uSelectedCat : undefined),
          applyDirectly: true
        })
      });
      ...
      if (data.success) {
        setM3uStatus({
          success: true,
          msg: `${data.message} Catalogue total : ${data.totalCatalog} chaînes.`
        });
        // Reload channels from server
        try {
          const chRes = await fetch(`/channels.json?t=${Date.now()}`);
          if (chRes.ok) {
            const updatedChs = await chRes.json();
            if (Array.isArray(updatedChs)) {
              onUpdateChannels(updatedChs);
            }
          }
        } catch (_) {}
        setM3uUrlInput('');
        ...
        setM3uStatus({
          success: false,
          msg: data.error || "Erreur lors de l'importation de l'URL."
        });
      }
    } catch (err: any) {
      setM3uStatus({
        success: false,
        msg: "Erreur réseau lors de l'importation de l'URL : " + err.message
      });
    } finally {
  ...
  const handleImportAll14Presets = async () => {
    setIsImportingUrl(true);
    setImportProgressMsg(`Importation en masse des 14 flux IPTV-ORG en cours...`);
    setM3uStatus(null);

    try {
      const urls = IPTV_ORG_PRESETS.map(p => p.url);
      const response = await fetch('/api/import-m3u-url', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.success) {
        setM3uStatus({
          success: true,
          msg: `🎉 Importation massive terminée ! ${data.added} nouvelles chaînes ajoutées, ${data.updated} mises à jour. Catalogue global : ${data.totalCatalog} chaînes.`
        });
        // Reload channels
        try {
          const chRes = await fetch(`/channels.json?t=${Date.now()}`);
        ...
        setM3uStatus({
          success: false,
          msg: data.error || "Erreur lors de l'importation globale."
        });
      }
    } catch (err: any) {
      setM3uStatus({
        success: false,
        msg: "Erreur réseau lors de l'importation globale : " + err.message
      });
    } finally {
      setIsImportingUrl(false);
      setImportProgressMsg('');
    ...
    <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-[1000] flex items-center justify-center p-4" id="admin-modal-pane">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6" id="admin-scroller-content">
          ...
          {activeSubTab === 'm3u' && (
            <div className="max-w-4xl mx-auto space-y-6 animate-fade-in" id="panel-admin-m3u-view">
              <div className="bg-[#0c101a] border border-white/5 rounded-2xl p-6 text-sm text-gray-300 space-y-3">
                <div className="flex items-center justify-between border-b border-white/15 pb-3">
                  <div className="flex items-center gap-2.5 text-[#e50914] font-bold uppercase tracking-wider">
                    ...
                    <span>Importateur Intelligent de Playlists IPTV M3U (Web & Local)</span>
                  </div>
                  <button
                    type="button"
                ...
                <p className="text-xs text-gray-400">
                  Ajoutez instantanément des milliers de chaînes via les <b>14 flux officiels IPTV-ORG</b> (Sports, Films, Actualités, Éducation, Météo, Arabe, etc.), depuis une URL M3U distante ou en collant du texte brut.
                </p>
              </div>

              {m3uStatus && (
                <div className={`p-4 rounded-xl text-xs border flex items-start gap-2.5 shadow-lg ${
                  m3uStatus.success 
              ...
              {/* SECTION 1: 14 OFFICIAL IPTV-ORG PLAYLISTS */}
              <div className="bg-[#0a0f1d] border border-white/10 rounded-2xl p-5 space-y-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
                  <div>
                    <h3 className="text-sm font-extrabold text-white flex items-center gap-2">
                      <span className="text-xl">📡</span>
                      <span>Les 14 Playlists Officielles IPTV-ORG</span>
                      <span className="text-[10px] bg-red-600/20 text-red-400 border border-red-500/30 px-2 py-0.5 rounded-full font-black">
                        14 FLUX PRÊTS
                      </span>
                    </h3>
                    <p className="text-[11px] text-gray-400 mt-0.5">
                      Cliquez sur une catégorie pour l&apos;importer, ou synchronisez l&apos;ensemble en 1 clic.
                    </p>
                  </div>

                  <button
                    type="button"
                    onClick={handleImportAll14Presets}
                    disabled={isImportingUrl}
                    className="px-4 py-2 bg-gradient-to-r from-red-600 to-rose-700 hover:from-red-500 hover:to-rose-600 text-white font-black rounded-xl text-xs tracking-wider shadow-lg shadow-red-600/30 flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50 shrink-0"
                  >
                    {isImportingUrl ? (
                      <>
                      ...
                      <>
                        ...
                        <span>Tout Importer (14 Playlists)</span>
                      </>
                    )}
                  </button>
                </div>
                ...
                {importProgressMsg && (
                  <div className="bg-red-950/40 border border-red-500/30 rounded-xl p-3 text-xs text-red-300 flex items-center gap-2 animate-pulse font-mono">
                    <RefreshCw className="w-3.5 h-3.5 animate-spin text-red-400" />
                    <span>{importProgressMsg}</span>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5">
                  {IPTV_ORG_PRESETS.map(preset => (
                    <button
                      ...
                      type="button"
                      disabled={isImportingUrl}
                      onClick={() => handleImportFromUrl(preset.url, preset.cat)}
                      className="bg-[#121829] hover:bg-[#1a233b] active:scale-95 border border-white/5 hover:border-red-500/40 rounded-xl p-2.5 text-left transition-all group flex flex-col justify-between gap-1 shadow-sm disabled:opacity-50"
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-lg group-hover:scale-110 transition-transform">{preset.icon}</span>
                        <span className="text-[9px] font-bold text-gray-400 group-hover:text-red-400 uppercase tracking-wider">
                          {preset.cat}
                        </span>
                      </div>
                      <div>
              ...
              {/* SECTION 2: IMPORT FROM CUSTOM URL */}
              <div className="bg-[#0a0f1d] border border-white/10 rounded-2xl p-5 space-y-4">
                <h3 className="text-sm font-extrabold text-white flex items-center gap-2">
                  <Globe className="w-4 h-4 text-cyan-400" />
                  <span>Importer depuis une URL M3U / M3U8 Personnalisée</span>
                </h3>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="sm:col-span-2">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">
                      URL du fichier .m3u ou .m3u8 :
                    </label>
                    <input
                      type="url"
                      placeholder="https://iptv-org.github.io/iptv/categories/sports.m3u"
                      value={m3uUrlInput}
                      onChange={(e) => setM3uUrlInput(e.target.value)}
                      className="w-full bg-[#101424] border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-red-500 font-mono"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      value={m3uSelectedCat}
                      onChange={(e) => setM3uSelectedCat(e.target.value)}
                      className="w-full bg-[#101424] border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-red-500"
                    >
                      <option value="AUTO">Automatique (group-title)</option>
                      {CATEGORIES.map(cat => (
                        <option key={cat} value={cat}>{cat}</option>
                      ))}
                    </select>
                ...
                <div className="flex justify-end">
                  <button
                    type="button"
                    disabled={isImportingUrl || !m3uUrlInput.trim()}
                    onClick={() => handleImportFromUrl()}
                    className="px-5 py-2.5 bg-cyan-600 hover:bg-cyan-500 active:scale-95 text-white font-bold rounded-xl text-xs tracking-wider shadow-lg shadow-cyan-600/30 flex items-center gap-2 transition-all disabled:opacity-50"
                  >
                    <Download className="w-4 h-4" />
                    <span>Télécharger & Importer cette URL</span>
                  </button>
                </div>
              </div>

              {/* SECTION 3: RAW TEXT M3U IMPORT */}
              <div className="bg-[#0a0f1d] border border-white/10 rounded-2xl p-5 space-y-3">
                <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest block">
                  Ou collez le script M3U brut (Texte) :
                </label>
                <textarea
                  rows={6}
                  ...
                  className="w-full bg-[#060813] border border-white/10 placeholder-gray-600 rounded-xl p-4 font-mono text-xs text-white focus:outline-none focus:border-red-600"
                />
                ...
                <div className="flex justify-end gap-3">
                  <button
                    type="button"
                    onClick={() => setRawM3u('')}
                    className="px-4 py-2 bg-neutral-800 hover:bg-neutral-700 text-gray-300 font-bold rounded-lg text-xs tracking-wider"
                  >
                    Vider le texte
                  </button>
                  <button
                    ...
                    className="px-5 py-2 bg-[#e50914] hover:bg-red-700 text-white font-bold rounded-lg text-xs tracking-wider shadow-lg shadow-red-600/30 flex items-center gap-2"
                  >
                    <Import className="w-4 h-4" />
                    Importer le texte brut
                  </button>
                </div>
              </div>
            </div>
                      ...
                      <div className="space-y-1.5 pt-2 border-t border-white/[0.05]">
                        ...
                        <div className="flex gap-2">
                          <input
                            type="text"
                            readOnly
                            value="https://www.tvpromedia.com/live/stream.m3u8"
                          ...
                          />
                          <button
                            onClick={() => copyToClipboard('https://www.tvpromedia.com/live/stream.m3u8', 'hls')}
                      ...
                      onChange={(e) => {
                        ...
                        if (targetId === 'studio') {
                          ...
                          if (targetCh) {
                            ...
                            setGenM3u8Url(targetCh.lien || `https://www.tvpromedia.com/live/${k}.m3u8`);
                          }
                        }
                      }}
                      ...
                      <div className="relative flex-1">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setGenM3u8Url(`https://www.tvpromedia.com/live/${e.target.value}.m3u8`);
                          }}
                  ...
                  <div className="space-y-2 pt-2 border-t border-white/5">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const freshUrl = `https://www.tvpromedia.com/live/${genStreamKey}.m3u8`;
...
server {
    ...
    location /live/ {
        proxy_pass https://www.tvpromedia.com/live/;
    ...
    location / {
        proxy_pass https://www.tvpromedia.com:3000;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
            ...
            <div className="space-y-6 animate-fade-in" id="panel-admin-sync-view">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-[#0b101d] border border-amber-500/30 rounded-xl p-4 space-y-2 relative overflow-hidden">
                  ...
                  <div className="pt-2 border-t border-white/5 flex items-center justify-between text-[10px]">
                    <span className="text-gray-400">SRS HLS HTTPS :</span>
                    <span className="text-emerald-400 font-mono">https://www.tvpromedia.com/live/</span>
                  </div>
                </div>
              ...
              <div className="bg-[#0e1424] border border-white/[0.08] rounded-2xl p-6 space-y-4 shadow-xl">
                ...
                <div className="space-y-4">
                  {/* Instant 1-sec channels deploy */}
                  <div className="bg-black/60 border border-emerald-500/40 rounded-xl p-4 space-y-2">
                    <div className="flex items-center justify-between text-xs text-gray-400">
                      <span className="flex items-center gap-1.5 font-bold text-emerald-400">
                        <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                        Déploiement direct &amp; instantané des 316 chaînes vers www.tvpromedia.com :
                      </span>
                      <button
                        type="button"
                        onClick={() => handleCopyText('curl -sSL https://tvpromedia.ai.studio/api/deploy-channels | bash', 'cmd-deploy-channels')}
                      ...
                      >
                        {copiedSyncCmd === 'cmd-deploy-channels' ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                        <span>{copiedSyncCmd === 'cmd-deploy-channels' ? 'Copié !' : 'Copier'}</span>
                      </button>
                    </div>
                    <pre className="text-xs text-emerald-300 font-mono bg-black/80 p-3 rounded-lg overflow-x-auto select-all border border-emerald-500/20">
                      curl -sSL https://tvpromedia.ai.studio/api/deploy-channels | bash
                    </pre>
                    <p className="text-[11px] text-gray-400 leading-relaxed">
                      Écrit immédiatement l&apos;ensemble des 316 chaînes (dont <strong className="text-white">MSTV</strong> et <strong className="text-white">MS RADIO</strong>) dans <code className="text-white">/var/www/tvpromedia/public/channels.json</code> et <code className="text-white">dist/channels.json</code>, et recharge PM2 et Nginx en 1 seconde.
                    </p>
                  </div>
                  ...
                  <div className="bg-black/60 border border-white/10 rounded-xl p-4 space-y-2">
                    <div className="flex items-center justify-between text-xs text-gray-400">
                      <span className="flex items-center gap-1.5">
                        ...
                        Déploiement complet (Code source, Nginx, SRS, PM2) :
                      </span>
                      <button
                        type="button"
                    ...
                    <p className="text-[11px] text-gray-400 leading-relaxed">
                      Télécharge le code complet de l&apos;application, compile avec <code className="text-white">npm run build</code>, configure <strong className="text-white">Nginx</strong> pour <span className="text-cyan-300 font-mono">tvpromedia.com</span> et <span className="text-cyan-300 font-mono">www.tvpromedia.com</span>, et redémarre avec <strong className="text-white">PM2</strong>.
                    </p>
                  </div>
                </div>
              </div>
