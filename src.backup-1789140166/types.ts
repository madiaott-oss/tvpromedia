...
export interface Channel {
  ...
  url?: string; // Stream URL alias
  src?: string; // Stream source alias
  direct?: boolean; // Live status indicator
  auditeurs?: number; // Current listeners / viewers count
...
/**
 * Resolves the active streaming URL for a channel by prioritizing:
 * const streamUrl = channel.lien || channel.m3u8Source || channel.url || channel.src;
 */
export function getChannelStreamUrl(channel?: Partial<Channel> | null): string {
  if (!channel) return '';
  const streamUrl = channel.lien || channel.m3u8Source || channel.url || channel.src || '';
  return streamUrl;
}
